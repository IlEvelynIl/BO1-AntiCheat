...
namespace anticheat {
    namespace integrity {
        ...
        std::string GetModifiedPlayerStates()
        {
            ...
            for (int i = 0; i <= 3; i++)
            {
                int demiGodMode = utils::memory::ReadInt(handle, playerStats[i]) & 2;
                int godMode = utils::memory::ReadInt(handle, playerStats[i]) & 1;
                int noTarget = utils::memory::ReadInt(handle, playerStats[i]) & 4;
